  ...
  return (
    <div className="rounded-xl border border-rw-line bg-rw-surfaceAlt p-3 space-y-2">
      ...
      <ul className="space-y-0.5">
        ...
        {pizzaItems.length === 0 && (
          <li className="text-xs text-rw-muted italic">{t("pizzeria.kds.no_items")}</li>
        )}
      </ul>
...
function NoteVocaliTab() {
  ...
  useEffect(() => {
    operationalNotesApi
      .list(AREA)
      .then(setNotes)
      .catch((e) => setError(e instanceof Error ? e.message : t("pizzeria.note.error_load")))
  ...
  async function save() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("pizzeria.note.error_save"));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function remove(id: string) {
    try {
      ...
      setError(e instanceof Error ? e.message : t("pizzeria.note.error_delete"));
    }
  }

  return (
    <div className="space-y-6">
      <Card title={t("pizzeria.note.new_title")} headerRight={<Mic className="h-5 w-5 text-rw-accent" />}>
        <div className="space-y-4">
          <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder={t("pizzeria.note.placeholder")} rows={4} className="w-full rounded-xl border border-rw-line bg-rw-bg px-4 py-2.5 text-sm text-rw-ink placeholder:text-rw-muted focus:outline-none focus:ring-1 focus:ring-rw-accent" />
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => void save()} disabled={saving || !text.trim()} className="flex items-center gap-2 rounded-xl bg-rw-accent px-5 py-2.5 text-sm font-bold text-white transition hover:bg-rw-accent/85 disabled:opacity-50">
              ...
              {saving ? t("pizzeria.saving") : t("pizzeria.note.save_btn")}
            </button>
            <button type="button" onClick={() => setText("")} className="rounded-xl border border-rw-line px-5 py-2.5 text-sm font-semibold text-rw-muted transition hover:text-rw-ink">{t("pizzeria.note.clear_btn")}</button>
          </div>
        </div>
      </Card>

      <Card title={t("pizzeria.note.history_title").replace("{n}", String(notes.length))} description={t("pizzeria.note.history_desc")}>
        {loading && <p className="py-4 text-center text-sm text-rw-muted">{t("ui.loading")}</p>}
        {!loading && notes.length === 0 && <p className="py-4 text-center text-sm text-rw-muted">{t("pizzeria.note.empty")}</p>}
...
export function PizzeriaPage() {
  ...
  const translatedTabs = TABS.map((tab) => ({
    ...tab,
    label: t(`pizzeria.tab.${tab.id}`),
  }));

  return (
    <div className="space-y-6">
      <PageHeader title={t("nav.pizzeria.label")} subtitle={t("pizzeria.subtitle")}>
        <Chip label={t("pizzeria.kds.chip_orders")} value={pizzeriaOrders.length} tone="info" />
        <Chip label={t("pizzeria.kds.chip_in_prep")} value={classified.inPrep.length} tone="accent" />
        <Chip label={t("pizzeria.kds.chip_pronti")} value={classified.pronti.length} tone="success" />
        <Chip label={t("pizzeria.kds.chip_late")} value={lateCount} tone={lateCount > 0 ? "danger" : "default"} />
      </PageHeader>
      ...
      {activeTab === "comande" && (
        <div className="grid gap-4 lg:grid-cols-3">
          <KdsColumn title={t("pizzeria.kds.col_waiting")} tone="pending" count={classified.inAttesa.length}>
            ...
            {classified.inAttesa.length === 0 && <p className="py-6 text-center text-xs text-rw-muted">{t("pizzeria.kds.empty_waiting")}</p>}
          </KdsColumn>

          <KdsColumn title={t("pizzeria.kds.col_in_prep")} tone="prep" count={classified.inPrep.length}>
            ...
            {classified.inPrep.length === 0 && <p className="py-6 text-center text-xs text-rw-muted">{t("pizzeria.kds.empty_in_prep")}</p>}
          </KdsColumn>

          <KdsColumn title={t("pizzeria.kds.col_ready")} tone="ready" count={classified.pronti.length}>
            ...
            {classified.pronti.length === 0 && <p className="py-6 text-center text-xs text-rw-muted">{t("pizzeria.kds.empty_ready")}</p>}
          </KdsColumn>
        </div>
      )}
