...
const HOTEL_AREAS = [
  { value: "Tutte", label: "Tutte le aree" },
...
type HotelArea = (typeof HOTEL_AREAS)[number]["value"];

/* ─── Shared constants (same as restaurant turni) ─── */
...
const SYNC_ROLES = new Set(["supervisor", "owner", "super_admin", "hotel_manager"]);
...
function formatDayLabel(iso: string) { const d = isoToDate(iso); return `${DAYS_IT[d.getDay()]} ${d.getDate()}`; }
function shiftTypeStyle(t: ShiftPlanType) { return SHIFT_TYPES.find((s) => s.value === t)?.color ?? SHIFT_TYPES[0].color; }
function shiftTypeLabel(t: ShiftPlanType) { return SHIFT_TYPES.find((s) => s.value === t)?.label ?? t; }
function computeHours(p: ShiftPlan): number {
  if (p.shiftType !== "lavoro") return 0;
...
function ShiftModal({ open, onClose, staff, initial, onSave, editId }: ModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setDay(initial?.day ?? toIso(new Date())); setArea(initial?.area ?? "reception");
      setStaffId(initial?.staffId ?? ""); setStaffName(initial?.staffName ?? "");
  ...
  function onStaffChange(id: string) {
    setStaffId(id);
    const m = hotelStaff.find((s) => s.id === id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-rw-line bg-rw-bg shadow-2xl">
        ...
        <div className="space-y-4 p-6">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select value={area} onChange={(e) => setArea(e.target.value)} className={inputCls}>
                {HOTEL_AREAS.filter((a) => a.value !== "Tutte").map((a) => (
          ...
          <div>
            <label className={labelCls}>Dipendente hotel</label>
            {hotelStaff.length > 0 && (
              <select value={staffId} onChange={(e) => onStaffChange(e.target.value)} className={inputCls}>
                <option value="">— scegli dallo staff hotel —</option>
                {hotelStaff.map((s) => (
                  <option key={s.id} value={s.id}>{s.name} ({hotelRoleLabel(s.role)})</option>
                ))}
              </select>
            )}
            {!staffId && (
              <input value={staffName} onChange={(e) => setStaffName(e.target.value)} placeholder="Nome operatore (libero)" className={cn(inputCls, hotelStaff.length > 0 && "mt-2")} />
            )}
            {staffId && (
              <button type="button" onClick={() => { setStaffId(""); setStaffName(""); }} className="mt-1 text-xs text-rw-accent hover:underline">Inserisci manualmente</button>
            )}
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {SHIFT_TYPES.map((t) => (
                <button key={t.value} type="button" onClick={() => setShiftType(t.value)}
                  className={cn("rounded-lg border px-3 py-1.5 text-xs font-semibold transition", shiftType === t.value ? t.color : "border-rw-line text-rw-muted hover:border-rw-accent/40")}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          {shiftType === "lavoro" && (
            <div className="grid gap-4 sm:grid-cols-2">
              <div><label className={labelCls}>Inizio turno</label><input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className={inputCls} /></div>
...
type WeekViewProps = {
  weekStart: Date; plans: ShiftPlan[]; filterArea: HotelArea;
  canEdit: boolean; onAdd: (day: string, area?: string) => void;
  onEdit: (p: ShiftPlan) => void; onDelete: (id: string) => void;
};

function WeekView({ weekStart, plans, filterArea, canEdit, onAdd, onEdit, onDelete }: WeekViewProps) {
  const days = Array.from({ length: 7 }, (_, i) => toIso(addDays(weekStart, i)));
  const filtered = plans.filter((p) => days.includes(p.day) && (filterArea === "Tutte" || p.area === filterArea));
  const grouped = filtered.reduce<Record<string, ShiftPlan[]>>((acc, p) => {
    if (!acc[p.day]) acc[p.day] = [];
    acc[p.day].push(p); return acc;
  }, {});
        ...
        {days.map((day) => {
          ...
          return (
            <div key={day} className="flex flex-col gap-1.5">
              <div className={cn("rounded-xl py-2 text-center text-xs font-bold", isToday ? "bg-rw-accent text-white" : "bg-rw-surfaceAlt text-rw-muted")}>
                {formatDayLabel(day)}
              </div>
              <div className="flex min-h-[180px] flex-col gap-1 rounded-xl border border-rw-line bg-rw-bg p-1.5">
                {dayPlans.map((p) => (
                  <div key={p.id} className={cn("group relative rounded-lg border px-2 py-1.5 text-[11px]", shiftTypeStyle(p.shiftType))}>
                    ...
                    {canEdit && (
                      <div className="absolute right-1 top-1 hidden group-hover:flex gap-0.5">
                        <button type="button" onClick={() => onEdit(p)} className="rounded p-0.5 hover:bg-white/20"><Edit2 className="h-2.5 w-2.5" /></button>
                        <button type="button" onClick={() => onDelete(p.id)} className="rounded p-0.5 hover:bg-red-400/30"><Trash2 className="h-2.5 w-2.5" /></button>
                      </div>
                    )}
                  </div>
                ))}
                {canEdit && (
                  <button type="button" onClick={() => onAdd(day, filterArea !== "Tutte" ? filterArea : undefined)}
...
type MonthViewProps = {
  year: number; month: number; plans: ShiftPlan[]; filterArea: HotelArea;
  canEdit: boolean; onAdd: (day: string) => void;
  onEdit: (p: ShiftPlan) => void; onDelete: (id: string) => void;
};
...
function MonthView({ year, month, plans, filterArea, canEdit, onAdd, onEdit, onDelete }: MonthViewProps) {
  ...
  const filtered = plans.filter((p) => {
    ...
    return d.getFullYear() === year && d.getMonth() === month && (filterArea === "Tutte" || p.area === filterArea);
  });
  const byDay = filtered.reduce<Record<number, ShiftPlan[]>>((acc, p) => {
    const d = isoToDate(p.day).getDate();
    if (!acc[d]) acc[d] = []; acc[d].push(p); return acc;
  }, {});
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {Array.from({ length: rows * 7 }, (_, i) => {
          ...
          if (dayNum < 1 || dayNum > lastDay.getDate()) return <div key={i} className="min-h-[80px] rounded-xl bg-rw-surfaceAlt/30" />;
          const iso = `${year}-${String(month + 1).padStart(2, "0")}-${String(dayNum).padStart(2, "0")}`;
          const dayPlans = byDay[dayNum] ?? [];
          const isToday = today.getFullYear() === year && today.getMonth() === month && today.getDate() === dayNum;
          return (
            <div key={i} className={cn("group flex min-h-[80px] flex-col gap-1 rounded-xl border p-1.5", isToday ? "border-rw-accent/50 bg-rw-accent/5" : "border-rw-line bg-rw-bg")}>
              <div className={cn("flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold self-end", isToday ? "bg-rw-accent text-white" : "text-rw-muted")}>{dayNum}</div>
              <div className="flex flex-col gap-0.5 flex-1">
                {dayPlans.slice(0, 3).map((p) => (
                  <div key={p.id} className={cn("flex items-center justify-between rounded px-1.5 py-0.5 text-[10px] border", shiftTypeStyle(p.shiftType))}>
                    <span className="truncate font-semibold">{p.staffName}</span>
                    {canEdit && (
                      <span className="hidden group-hover:flex gap-0.5 ml-1 shrink-0">
                        <button onClick={() => onEdit(p)}><Edit2 className="h-2 w-2" /></button>
                        <button onClick={() => onDelete(p.id)}><Trash2 className="h-2 w-2" /></button>
                      </span>
                    )}
                  </div>
                ))}
                {dayPlans.length > 3 && <div className="text-[10px] text-rw-muted pl-1">+{dayPlans.length - 3} altri</div>}
              </div>
              {canEdit && (
                <button type="button" onClick={() => onAdd(iso)}
...
type SummaryProps = { plans: ShiftPlan[]; staff: StaffMember[]; filterArea: HotelArea; from: string; to: string };

function SummaryView({ plans, staff, filterArea, from, to }: SummaryProps) {
  const filtered = plans.filter((p) => p.day >= from && p.day <= to && (filterArea === "Tutte" || p.area === filterArea));
  type Stat = { name: string; role: string; hoursLavoro: number; daysLavoro: number; daysFerie: number; daysMalattia: number; daysPermesso: number; daysRiposo: number };
  const map = new Map<string, Stat>();
  for (const p of filtered) {
    const key = p.staffId ?? p.staffName;
    if (!map.has(key)) {
      const m = staff.find((s) => s.id === p.staffId || s.name === p.staffName);
      map.set(key, { name: p.staffName, role: m?.role ?? p.role ?? "", hoursLavoro: 0, daysLavoro: 0, daysFerie: 0, daysMalattia: 0, daysPermesso: 0, daysRiposo: 0 });
    }
    const stat = map.get(key)!;
    if (p.shiftType === "lavoro") { stat.hoursLavoro += computeHours(p); stat.daysLavoro++; }
    else if (p.shiftType === "ferie") stat.daysFerie++;
    else if (p.shiftType === "malattia") stat.daysMalattia++;
    else if (p.shiftType === "permesso") stat.daysPermesso++;
    else if (p.shiftType === "riposo") stat.daysRiposo++;
  }
  const stats = Array.from(map.values()).sort((a, b) => a.name.localeCompare(b.name));
  if (stats.length === 0) return <p className="py-10 text-center text-sm text-rw-muted">Nessun turno nel periodo selezionato.</p>;
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[640px] text-sm">
        <thead>
          <tr className="border-b border-rw-line text-xs font-semibold text-rw-muted">
            <th className="py-3 text-left pl-2">Dipendente</th><th className="py-3 text-left">Ruolo</th>
            <th className="py-3 text-right">Ore lavorate</th><th className="py-3 text-right">Gg lavoro</th>
            <th className="py-3 text-right">Ferie</th><th className="py-3 text-right">Malattia</th>
            <th className="py-3 text-right">Permesso</th><th className="py-3 text-right pr-2">Riposo</th>
          </tr>
        </thead>
        <tbody>
          {stats.map((s, i) => (
            <tr key={i} className="border-b border-rw-line/40 hover:bg-rw-surfaceAlt/40 transition">
              ...
              <td className="py-2.5 text-rw-muted text-xs">{hotelRoleLabel(s.role)}</td>
              <td className="py-2.5 text-right font-mono">{s.hoursLavoro > 0 ? `${s.hoursLavoro.toFixed(1)}h` : "—"}</td>
...
export function HotelTurniPage() {
  ...
  const [filterArea, setFilterArea] = useState<HotelArea>("Tutte");
  const now = new Date();
  ...
  function openAdd(day?: string, area?: string) {
    setEditId(undefined);
    setModalInitial({ day: day ?? toIso(new Date()), area: area ?? (filterArea !== "Tutte" ? (filterArea as string) : "reception") });
    setModalOpen(true);
  }

  function openEdit(plan: ShiftPlan) { setEditId(plan.id); setModalInitial(plan); setModalOpen(true); }
  ...
  async function handleSync() {
    if (!confirm("Sincronizzare lo stato del personale hotel in base ai turni di oggi?")) return;
    setSyncing(true); setSyncMsg(null);
    try {
      const result = await shiftPlansApi.sync(weekFrom, weekTo);
      setSyncMsg(`Sincronizzati ${result.updated.length} dipendenti su ${result.summary.totalShifts} turni.`);
    } catch (e) { setSyncMsg(e instanceof Error ? e.message : "Errore"); }
    finally { setSyncing(false); }
  }

  const visiblePlans = useMemo(() => plans.filter((p) => filterArea === "Tutte" || p.area === filterArea), [plans, filterArea]);

  return (
    <div className="space-y-6">
      <PageHeader title="Turni Hotel" subtitle="Pianificazione settimanale e mensile del personale alberghiero.">
        {canSync && (
      ...
      {syncMsg && (
        <div className="flex items-center gap-3 rounded-xl border border-rw-accent/30 bg-rw-accent/10 px-4 py-3 text-sm text-rw-accent">
          <CalendarClock className="h-4 w-4 shrink-0" />{syncMsg}
          <button type="button" onClick={() => setSyncMsg(null)} className="ml-auto"><X className="h-4 w-4" /></button>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap gap-1 ml-auto">
          {HOTEL_AREAS.map((a) => (
            <button key={a.value} type="button" onClick={() => setFilterArea(a.value as HotelArea)}
              className={cn("rounded-xl px-3 py-2 text-xs font-semibold transition",
                filterArea === a.value ? "bg-rw-accent/15 text-rw-accent" : "border border-rw-line text-rw-muted hover:text-rw-ink")}>
              {a.label}
            </button>
          ))}
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16 text-rw-muted"><Loader2 className="h-6 w-6 animate-spin mr-3" /> Caricamento turni…</div>
      ) : (
        <>
          {tab === "settimana" && (
            <Card
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <button type="button" onClick={() => setWeekStart((d) => addDays(d, -7))} className={btnGhost}><ChevronLeft className="h-4 w-4" /></button>
                <button type="button" onClick={() => setWeekStart(getWeekStart(new Date()))} className="text-xs text-rw-accent hover:underline">Oggi</button>
                <button type="button" onClick={() => setWeekStart((d) => addDays(d, 7))} className={btnGhost}><ChevronRight className="h-4 w-4" /></button>
              </div>
              <WeekView weekStart={weekStart} plans={plans} filterArea={filterArea} canEdit={canEdit}
                onAdd={(day, area) => openAdd(day, area)} onEdit={openEdit} onDelete={(id) => void handleDelete(id)} />
            </Card>
          )}
          {tab === "mese" && (
            <Card
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <button type="button" onClick={() => setMonthYear(({ year, month }) => month === 0 ? { year: year - 1, month: 11 } : { year, month: month - 1 })} className={btnGhost}><ChevronLeft className="h-4 w-4" /></button>
                <button type="button" onClick={() => setMonthYear({ year: now.getFullYear(), month: now.getMonth() })} className="text-xs text-rw-accent hover:underline">Mese corrente</button>
                <button type="button" onClick={() => setMonthYear(({ year, month }) => month === 11 ? { year: year + 1, month: 0 } : { year, month: month + 1 })} className={btnGhost}><ChevronRight className="h-4 w-4" /></button>
              </div>
              <MonthView year={monthYear.year} month={monthYear.month} plans={plans} filterArea={filterArea} canEdit={canEdit}
                onAdd={(day) => openAdd(day)} onEdit={openEdit} onDelete={(id) => void handleDelete(id)} />
            </Card>
          )}
          {tab === "riepilogo" && (
            <Card title="Riepilogo ore e presenze hotel" description={`${monthFrom} → ${monthTo}`}>
              <SummaryView plans={plans} staff={staff} filterArea={filterArea} from={monthFrom} to={monthTo} />
            </Card>
          )}
        </>
      )}
