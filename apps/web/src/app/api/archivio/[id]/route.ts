...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  const i = await operationsRepository.archivio.get(getTenantId(), id);
...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  const up = await operationsRepository.archivio.update(getTenantId(), id, u);
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  if (guard.error) return guard.error;
  const { id } = await ctx.params;
  const deleted = await operationsRepository.archivio.delete(getTenantId(), id);
